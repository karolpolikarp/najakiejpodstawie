    ...
    if (tool.name === 'search_legal_info') {
      ...
      if (detectedTopics.length === 0) {
        return {
          type: 'tool_result',
          tool_use_id: tool.id,
          content: `Nie znaleziono tego tematu w bazie wiedzy kontekstowej.

INSTRUKCJA: Odpowiedz na pytanie używając swojej wiedzy prawniczej. Wytyczne:
1. Zidentyfikuj odpowiednie artykuły z kodeksów (KC, KK, KP, KPK, KPC, KRO, KPA, etc.)
2. Podaj ogólną odpowiedź z numerami artykułów
3. Dodaj na końcu ostrzeżenie: "⚠️ Odpowiedź oparta na wiedzy AI - zweryfikuj na https://isap.sejm.gov.pl"
4. Format odpowiedzi: standardowy (PODSTAWA PRAWNA, CO TO OZNACZA, POWIĄZANE PRZEPISY, UWAGA)

Nie używaj więcej narzędzi - odpowiedz bezpośrednio.`,
          is_error: false
        };
      }
